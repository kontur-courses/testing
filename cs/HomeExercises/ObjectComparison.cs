...
namespace HomeExercises
{
    public class ObjectComparison
    {
        ...
        public void CheckCurrentTsar_WithCustomEquality()
        {
            ...
            // Недостатки:
            // 1)  Тест не скажет, на каком поколении, в каком поле произошла ошибка,
            // а просто укажет, что ошибка в строчке Assert.True(AreEqual(actualTsar, expectedTsar));

	        // 2) При расширении функциональности придется добавлять проверку для новых полей.

	        // 3) Логика работы теста не очевидна, придется смотреть реализацию.

            // Данное решение лучше тем, что FluentAssertions явно укажет на те поля, которые не совпадают. 
